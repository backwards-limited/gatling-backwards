...
class Simulation6SetupOpenSystem extends Simulation {
  ...
  val csvFeeder: SourceFeederBuilder[String] =
    csv("data/passengers.csv").circular

  val randomEmailFeeder: Iterator[Map[String, String]] =
    Iterator.continually(Map("randomEmail" -> (Random.alphanumeric.take(20).mkString + "@ggmail.com")))

  val scenario1: ScenarioBuilder = scenario("SE1")
    .exec(http("Homepage")
      .get("/")
      .headers(headers_0)
  ...
  val scenario2: ScenarioBuilder = scenario("SE2")
    .exec(flushHttpCache)
  ...
  // Control arrival rate of users but not the actual number.
  setUp(
    scenario1.inject(constantUsersPerSec(2) during(10 seconds)), // 2 users join every second over 10 seconds
    scenario2.inject(constantUsersPerSec(1) during(10 seconds))  // 1 user joins every second over 10 seconds
